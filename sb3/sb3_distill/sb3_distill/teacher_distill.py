...
class TeacherDistill(PolicyDistillationAlgorithm, OnPolicyAlgorithm):
    """
    ...
    The rollout buffer is collected using the *teacher* policy.

    Note: in this implementation of teacher-distillation the value
    ...
    def learn(
        ...
        tb_log_name: str = "teacher-distill",
    ...
    def collect_rollouts(
        ...
        self.student_policy = self.policy

        if hasattr(self, 'teacher_model') and self.teacher_model is not None:
            # Set the teacher policy to be the policy used to collect the rollout buffer
            self.policy = self.teacher_model.policy
        else:
            print("ERROR: must first call model.set_teacher(teacher_model)")

        ret = super().collect_rollouts(env=env,
                                       callback=callback,
