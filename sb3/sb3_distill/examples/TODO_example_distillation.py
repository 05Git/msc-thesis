...
if __name__ == "__main__":
    env = make_atari_env(f"{atari_env}NoFrameskip-v4", n_envs=16, seed=100, vec_env_cls=SubprocVecEnv, wrapper_kwargs=dict(clip_reward=False, terminal_on_life_loss=True) )
    ...
    if distill_method == 'PPD':
        ...
        student_model = ProximalPolicyDistillation("CnnPolicy", env, verbose=1, policy_kwargs=largenet_policy_kwargs, n_steps=256, batch_size=512, n_epochs=4, learning_rate=3e-4, gamma=0.995, ent_coef=0.01, tensorboard_log="tensorboard/")
        ...
        student_model = StudentDistill("CnnPolicy", env, verbose=1, policy_kwargs=largenet_policy_kwargs, n_steps=5, learning_rate=3e-4, gamma=0.999, ent_coef=0.01, tensorboard_log="tensorboard/")
        ...
        student_model = TeacherDistill("CnnPolicy", env, verbose=1, policy_kwargs=largenet_policy_kwargs, n_steps=n_steps, learning_rate=3e-4, gamma=0.999, ent_coef=0.01, tensorboard_log="tensorboard/")
        student_model.set_teacher(teacher_model)


    student_model.learn(total_timesteps=distill_timesteps, tb_log_name=atari_env+'_'+distill_method)  # TODO: run and see if 1m was enough!

    # TODO: test new 'exclude teacher_model' in base class!
    ##student_model.set_teacher(None) # required for saving
    student_model.save(f'nn/{atari_env}_student_'+distill_method+'.ckpt')

    mean_reward, std_reward = evaluate_policy(teacher_model, teacher_model.get_env(), n_eval_episodes=10, deterministic=True)
    print('Raw teacher score: ', mean_reward, '+-', std_reward) # 500k : -188 +- 119
    print('Human-normalized mean teacher score: ', normalize_score(mean_reward, atari_env))

    mean_reward, std_reward = evaluate_policy(student_model, student_model.get_env(), n_eval_episodes=10, deterministic=True)
